 ...
 * one object instance.
 */
void createAdvancedUI(Grid grid, ref uint row, Settings delegate() scb, bool showTriggerLineSettings = false) {
    ...
    btnEditLink.connectClicked(delegate(Button b) {
        Settings gs = scb();
        ...
        dlg.showAll();
        if (dlg.run() == gtk.types.ResponseType.Apply) {
    ...
    if (checkVTEFeature(TerminalFeature.EVENT_SCREEN_CHANGED)) {
        ...
        btnEditTriggers.connectClicked(delegate(Button b) {
            Settings gs = scb();
