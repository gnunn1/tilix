...
private:
	...
	void createUI() {
        ...
        ComboBox cbCursorShape = createNameValueCombo([_("Block"), _("IBeam"),_("Underline") ], [SETTINGS_PROFILE_CURSOR_SHAPE_BLOCK_VALUE,SETTINGS_PROFILE_CURSOR_SHAPE_IBEAM_VALUE,SETTINGS_PROFILE_CURSOR_SHAPE_UNDERLINE_VALUE]);
		...
		gsProfile.bind(SETTINGS_PROFILE_USE_SYSTEM_FONT_KEY, fbFont, "sensitive",
...
private:
	...
	void createUI() {
        ...
        cbUseThemeColors.addOnToggled(delegate(ToggleButton) {
            setCustomScheme();
        });
	...
	Grid createColorGrid(int row) {
		...
		gsProfile.bind(SETTINGS_PROFILE_USE_THEME_COLORS_KEY, cbBG, "sensitive",
			GSettingsBindFlags.GET | GSettingsBindFlags.NO_SENSITIVITY | GSettingsBindFlags.INVERT_BOOLEAN);
		...
		gsProfile.bind(SETTINGS_PROFILE_USE_THEME_COLORS_KEY, cbFG, "sensitive",
			GSettingsBindFlags.GET | GSettingsBindFlags.NO_SENSITIVITY | GSettingsBindFlags.INVERT_BOOLEAN);
    ...
    void initColorSchemeCombo() {
        ...
        if (index<0) cbScheme.setActive(to!int(schemes.length));
        else cbScheme.setActive(index);        
    }
    ...
    void setColorScheme(ColorScheme scheme) {
        ...
        scope(exit) {schemeChangingLock = false;}
